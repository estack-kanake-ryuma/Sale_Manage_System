 ...
 */
class Depart_sum_on_pdf extends Pdf_common {
        
        private $m_list_st_y = 65;
        private $m_break_page_cnt = 30;
        private $m_list_st_x = 15;
        
        private $al_last_no_tax = array();                      // 前年同期得意先計
        private $al_now_no_tax = array();                       // 当期得意先計
        private $al_diff_no_tax = array();                      // 増減得意先計
	...
	*/
	...
	public function display($data)
	{
            ...
            // 明細作成
            $this->_set_depart_on_spec($data);
            
            // PDF出力終了処理
            $from = $this->get_search_condition('book_date_from');
            $to = $this->get_search_condition('book_date_to');
            
            $fname = 'uriagesyukei_bumon_tekiyouari_'.str_replace("/", "", $from).'_'.str_replace("/", "", $to);
	 ...
	 * @author Kita Yasuhiro
	 */
        private function _set_depart_on_spec($data) {
            
            // 表のタイトル行を設定
            $title_conf = $this->set_list_title($this->m_list_st_x, $this->m_list_st_y, "sales_sum_depart_on");
            ...
            foreach ($data as $key=>$val) {
                ...
                // 得意先名称を設定
                $this->_set_customer_ctrl($data, $key, $y);
            ...
            // 摘要名称を設定
            $this->_set_customer_ctrl($data, count($data), $y, true);
         ...
         * @author Kita Yasuhiro
         */
        private function _set_customer_ctrl($data, $key, &$y, $last_flg = false)
        {
            ...
            if($last_flg) {
                ...
                $this->_pdf->SetXY($this->m_list_st_x, $y+=$this->_list_height);
                $this->_pdf->Cell(80, $this->_list_height, $this->change_code("【".$data[$key-1]->customer_name."  計】 "), 1, "", "R");
                
                $this->_pdf->Cell(23, $this->_list_height, $this->change_code(money_sep(array_sum($this->al_last_no_tax))), 1, "", "R");
                $this->_pdf->Cell(23, $this->_list_height, $this->change_code(money_sep(array_sum($this->al_now_no_tax))), 1, "", "R");
                $this->_pdf->Cell(23, $this->_list_height, $this->change_code(money_sep(array_sum($this->al_diff_no_tax))), 1, "", "R");
                $this->_pdf->Cell(23, $this->_list_height,
                        $this->change_code(calc_inc_dec_rate(array_sum($this->al_now_no_tax), array_sum($this->al_last_no_tax))."%" ), 1, "", "R");
                
//                $this->_pdf->SetXY($this->m_list_st_x, $y+=7);
//                $this->_pdf->Cell(172, $this->_list_height, "", 1, "", "L");
            ...
            if($key == 0) {
                ...
                $this->_pdf->Cell(172, $this->_list_height, $this->change_code("● ".$data[$key]->customer_name), 1, "", "L");
                $this->_pdf->SetXY($this->m_list_st_x, $y+=7);
                
            } else {
                
                if($data[$key]->customer_name == $data[$key-1]->customer_name && $data[$key]->disp_depart_id == $data[$key-1]->disp_depart_id) {
...
//                    $this->_pdf->SetXY($this->m_list_st_x, $y+=7);
                    ...
                    $this->al_last_no_tax = array();
                    $this->al_now_no_tax = array();
                    $this->al_diff_no_tax = array();
	 ...
	 */
        private function _set_page_break($data, $key, &$y, $auto_break_flg=false)
        {
            ...
            if($this->m_list_cnt == $this->m_break_page_cnt || $auto_break_flg == true) {
                ...
                $this->set_list_title($this->m_list_st_x, $this->m_list_st_y, "sales_sum_depart_on");
	 ...
	 */
        private function _sum_money($info) {
            ...
            $this->al_last_no_tax[] = $info->bef_no_tax;
            $this->al_now_no_tax[] = $info->now_no_tax;
            $this->al_diff_no_tax[] = $info->diff_no_tax;
