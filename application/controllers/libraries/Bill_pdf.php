...
//            $this->_set_spec_line($title_conf);
            ...
            foreach ($data as $val) {
                ...
                $row_length = (substr_count($val->goods_name, "\n") * $this->_list_height) + $this->_list_height;
                ...
                // 改ページ
                $cnt++;
                if($y + $row_length >= $this->m_max_page_length) {
                    $cnt =0;
                ...
                for($i=0; $i<count($title_conf); $i++) {
                    ...
                    if($title_conf[$i]['col_name'] == 'goods_name') {
                        ...
                        $this->_pdf->Cell($title_conf[$i]['width'], $row_length, $disp_str, 1, "", $title_conf[$i]['align']);
                    }
                ...
                if($this->_bill_type == "cutoff") {
                    if(empty($val->unit_memo)) {
                        $val->unit_memo = slash_date($val->book_date)." ご報告";
                    } else {
                        $val->unit_memo = slash_date($val->book_date)." ご報告\n".$val->unit_memo;
                    }
                ...
                if(!empty($val->unit_memo)) {
                    ...
                    $row_length = (substr_count($val->unit_memo, "\n") * $this->_list_height) + $this->_list_height;
                    ...
                    $this->_pdf->SetXY($this->m_st_x, $y);
                    $this->_pdf->MultiCell(189, $this->_list_height, $this->change_code($val->unit_memo), 1, "", "L");
	 ...
	 */
        private function _set_total_blk($data, $y, $cnt) {
            ...
            if($y + ((count($this->_al_tax_money) + 1) * 7)  >= $this->m_max_page_length) {
                $y = $this->m_list_st_y;
	 ...
	 * @author Kita Yasuhiro
	 */
//        private function _set_spec_line($title_conf)
//        {
//            if($this->_pdf->PageNo() > 1) return;
//            
//            $y = $this->m_list_st_y;
//            for($i=0; $i < 19; $i++) {
//                
//                $this->_pdf->SetXY($this->m_st_x, $y+=7);
//                 
//                for($j=0; $j <count($title_conf); $j++) {
//                    $this->_pdf->Cell($title_conf[$j]['width'], $title_conf[$j]['height'], "", 1, "", "C", 1);
//                }
//            }
//            
//        }
