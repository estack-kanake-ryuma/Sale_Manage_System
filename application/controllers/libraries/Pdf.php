	 ...
	 */
        private function _set_sales_spec($data) {
            ...
            foreach ($data as $key=>$val) {
                ...
                if(!empty($val->report_eda_from)) {
                    $data[$key]->report_no = $val->report_no.":".$val->report_eda_from."～".$val->report_eda_to;
                }
        ...
        private function _set_abstract_ctrl($data, $key, &$y, $last_flg = false)
        {
            ...
            if($last_flg) {
                ...
                $abstract = "【".$data[$key-1]->abstract_name."　計】";
                if(strstr($data[$key-1]->abstract_name, "受入収支")) {
                    $abstract = $abstract."    ";
                }
                
                $this->_pdf->SetXY($this->m_list_st_x, $y+=$this->_list_height);
                $this->_pdf->Cell(148, $this->_list_height, $this->change_code($abstract), 1, "", "R");
                $this->_pdf->Cell(23, $this->_list_height, $this->change_code(money_sep(array_sum($this->al_abstract_money))), 1, "", "R");
                $this->_pdf->Cell(20, $this->_list_height, "", 1, "", "");
                return;
            }
        ...
        private function _set_depart_ctrl($data, $key, &$y, $last_flg = false)
        {
            ...
            if($last_flg) {
                ...
                $this->_pdf->SetXY($this->m_list_st_x, $y+=$this->_list_height);
                $this->_pdf->Cell(148, $this->_list_height, $this->change_code("【".$data[$key-1]->depart_name    ."　計】"), 1, "", "R");
