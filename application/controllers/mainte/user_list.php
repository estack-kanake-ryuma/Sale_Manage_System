...
class User_list extends MY_Controller {
	...
	public function index()
	{
                ...
                // 検索条件を設定
                $condition = $this->user_mdl->create_search_condition();
		...
		// 検索条件の入力値
		$data['search'] = $this->user_mdl->get_input_condition();
                
                // システムユーザー一覧用のセッションを設定
                $this->user_mdl->set_list_session();

		$this->load->view('mainte/user_list', $data);
	}
	...
	public function delete()
	{
            ...
            // 削除処理実行
            $bRes = $this->user_mdl->delete_data($id);
	 ...
	 */
        private function _get_disp_list_data($condition) 
        {
            ...
            // 一覧を取得
            $list = $this->user_mdl->get_list_data($condition, $start);
	 ...
	 */
        private function _get_total_cnt($condition) {           
            ...
            // 件数取得
            return $this->user_mdl->get_total_cnt($condition);
